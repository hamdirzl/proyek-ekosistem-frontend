...
function attachSplitPdfListener() {
    ...
    const previewsContainer = document.getElementById('pdf-previews-container');
    const rangesContainer = document.getElementById('custom-ranges-container');
    const addRangeBtn = document.getElementById('add-range-btn');
    const splitBtn = document.getElementById('split-pdf-btn');
    const mergeCheckbox = document.getElementById('merge-ranges-checkbox');
    const messageDiv = document.getElementById('split-pdf-message');
    
    let pdfFile = null;
    let totalPages = 0;

    const renderPage = (pdf, pageNum, canvas) => {
    ...
    const addRange = () => {
        const rangeCount = rangesContainer.children.length + 1;
        const newRange = document.createElement('div');
        newRange.className = 'range-input-group';
        newRange.innerHTML = `
            <label>Range ${rangeCount}:</label>
            <input type="number" class="range-from" min="1" max="${totalPages}" value="1" required>
            <span>to</span>
            <input type="number" class="range-to" min="1" max="${totalPages}" value="${totalPages}" required>
            <button type="button" class="delete-range-btn">&times;</button>
        `;
        rangesContainer.appendChild(newRange);
        
        newRange.querySelector('.delete-range-btn').addEventListener('click', () => {
            newRange.remove();
            // Re-label ranges
            rangesContainer.querySelectorAll('.range-input-group').forEach((group, index) => {
                group.querySelector('label').textContent = `Range ${index + 1}:`;
            });
        });
    };
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        previewsContainer.innerHTML = '<p class="preview-placeholder">Rendering PDF previews...</p>';
        ...
        reader.onload = async (event) => {
            try {
                ...
                previewsContainer.innerHTML = ''; // Clear loading message
                
                for (let i = 1; i <= totalPages; i++) {
                    const previewWrapper = document.createElement('div');
                    previewWrapper.className = 'pdf-page-preview';
                    ...
                    previewWrapper.appendChild(pageNumLabel);
                ...
                previewsContainer.innerHTML = '<p class="preview-placeholder" style="color: red;">Gagal memuat pratinjau PDF. File mungkin rusak.</p>';
            }
        };
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const ranges = Array.from(rangesContainer.children).map(group => ({
            from: group.querySelector('.range-from').value,
            to: group.querySelector('.range-to').value
        }));

        if (ranges.length === 0) {
            messageDiv.textContent = 'Error: Please add at least one range.';
            messageDiv.className = 'error';
            return;
        }
        ...
        try {
            ...
            messageDiv.textContent = 'Success! Your file is downloading.';
            messageDiv.className = 'success';
